...
type Game struct {
	...
	instructions      []ebiten.Key
	pieceInstructions []ebiten.Key
}
...
func (g *Game) Update() error {
    ...
    if g.pieceCount == 2022 {
        g.state = GameStateGameOver
    }
	switch g.state {
	case GameStatePlaying:
		if g.elapsedDrop == g.dropTicks/2 {
			if len(g.pieceInstructions) == 0 {
				g.pieceInstructions = g.instructions
			}
			step := g.pieceInstructions[0]
			g.pieceInstructions = g.pieceInstructions[1:]
			g.processInput(step)
		}
...
func (g *Game) drawText(screen *ebiten.Image, gameZonePos *Position) {
	...
	text.Draw(screen, "MAX HEIGHT", g.txtFont, boardWidth+gameZonePos.X*2, gameZonePos.Y*4, color.White)
	text.Draw(screen, fmt.Sprintf("%08d", g.maxHeight()), g.txtFont, boardWidth+gameZonePos.X*2, gameZonePos.Y*4+8, color.White)
	...
	dy := 48
	text.Draw(screen, "NEXT", g.txtFont, boardWidth+gameZonePos.X*2, gameZonePos.Y*2+2*dy, color.White)
}
...
func NewGame(instructions []ebiten.Key) *Game {
	ebiten.SetMaxTPS(18)
	...
	game := &Game{
		...
		dropTicks:         2,
		pieces:            allPieces,
		gameZoneSize:      Size{Width: 7, Height: 5300},
		bgBlockImage:      createImage(imgBlockBG),
		instructions:      instructions,
		pieceInstructions: instructions,
	}
